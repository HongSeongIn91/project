...
public class ModifyArticleService {
  ...
  public void modify(ModifyRequest modReq) throws ClassNotFoundException, NamingException {
    ...
    try {
      ...
      // 게시글 번호에 해당하는 Article 객체를 구한다. 해당 번호를 가진 게시글이 존재하지 않으면 익셉션을 발생시킨다.
      Article article = articleDao.selectById(conn, modReq.getArticleNumber());
      if(article == null) {
        throw new ArticleNotFoundException();
      }
      // 수정하려는 사용자가 해당 게시글을 수정할 수 있는지 검사한다. 수정할 수 없으면 익셉션을 발생시킨다.
      if(!canModify(modReq.getUserId(), article)) {
        throw new PermissionDeniedException();
      }
      // 두 DAO의 update() 메서드를 이용해서 제목과 내용을 수정한다.
  ...
  // 게시글을 수정할 수 있는지 검사하는 기능을 구현한다. 수정하려는 사용자 ID가 게시글 작성자 ID와 동일하면 true를 리턴한다.
